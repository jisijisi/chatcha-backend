...
`,
  ...
  **🧠 REASONING & TRANSLATION PROCESS (CRITICAL):**
  ...
  4. **Nuance:** Ensure the Tagalog/Taglish translation feels natural and corporate.
`,
  SMART_DATA_ANALYST: `        You are a Smart Data Analyst for Live Data Tools.
...
`,
  SMART_PERSONAL_ASSISTANT: `        # SMART PERSONAL ASSISTANT ANALYSIS
...
export class PromptService {
   ...
   * Fetches from DB, caches it, falls back to default.
   */
  static async getPrompt(type) {
    ...
    try {
      ...
      const [rows] = await pool.execute(
        'SELECT system_prompt FROM ai_prompts WHERE type = ? AND is_active = TRUE',
        [type]
      );

      if (rows.length > 0) {
        const prompt = rows[0].system_prompt;
        this.cache.set(type, prompt);
        return prompt;
      } else {
        // 3. Fallback to default if DB missing
        console.warn(`⚠️ Prompt type '${type}' not found in DB. Using default.`);
  ...
  /**
   * Update a prompt in the DB and cache.
   */
  static async updatePrompt(type, content) {
    try {
      await pool.execute(
        'UPDATE ai_prompts SET system_prompt = ? WHERE type = ?',
        [content, type]
      );
      this.cache.set(type, content);
      console.log(`✅ Updated prompt '${type}'`);
      return true;
    } catch (error) {
  ...
  static async getAllPrompts() {
    try {
      const [rows] = await pool.execute(
        'SELECT type, description, system_prompt, updated_at FROM ai_prompts ORDER BY type'
      );
      return rows;
    } catch (error) {
